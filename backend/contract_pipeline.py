...
def select_agents_for_question(question: str) -> List[str]:
    ...
    if any(k in q for k in ["compliance", "complaince", "gdpr", "hipaa", "privacy", "data protection", "security", "breach", "incident", "retention", "subprocessor", "audit"]):
    ...
    # If the question doesn't map cleanly to an agent, default to legal.
    return selected or ["legal"]
...
def _agent_plan(agent_type: str, question: str) -> List[str]:
    # Very basic static planner to ensure we retrieve relevant clauses per domain.
    # If a specific question is provided, ONLY query that question.
    if question and question.strip():
        return [question.strip()]

    base = f"what are the {agent_type} terms?"
...
def run_agent(
    *,
    ...
    combined_text = " ".join([m.text for m in all_matches]).lower()
    
    # If a specific question was asked, verify the retrieved text is actually relevant to this agent's domain
    if question and question.strip():
        # Only run the agents that are directly relevant to this specific question topic
        relevant_agents = select_agents_for_question(question)
        if agent_type not in relevant_agents:
            return {
                ...
                "confidence": 0,
                "risk_level": "n/a",
                "retrieval": {"top_k_per_query": top_k_per_query, "per_query": per_query},
                "evidence": [],
                "findings": [f"This query is handled by the {relevant_agents[0].title()} agent. No specific {agent_type.title()} risk identified."],
                "skipped": False,
            }
    ...
    if question and question.strip():
        findings = [
            f"Evaluated the document specifically for '{question}'.",
            "Proceed with caution based on the retrieved clauses.",
        ]
    else:
        if agent_type == "legal":
            findings = [
            ...
            findings = [
                ...
                "Check SLA/uptime commitments and service credits.",
...
def build_executive_report_data(
    *,
    ...
    if question and question.strip():
        exec_points = [
            f"Analysis specifically focused on evaluating the document against the query: '{question}'.",
            "The contract text was scanned to extract clauses directly addressing this request.",
            f"Primary domains identified for this evaluation: {', '.join([s.title() for s in selected_set])}.",
            "Please review the direct Answer and detailed Agent Findings below for granular context."
        ]
    else:
        ...
        exec_points = [t for _, t in summary_candidates[:3]]
        if not exec_points:
            exec_points = ["No relevant risk-bearing clauses were identified in the provided text for the reviewed categories."]
            overall = "unknown"
    ...
    analysis = {
        ...
        "executive_summary_points": exec_points[:3],
